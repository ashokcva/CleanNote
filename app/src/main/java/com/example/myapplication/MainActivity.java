...
public class MainActivity extends AppCompatActivity implements PopupMenu.OnMenuItemClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        notesList = roomDB.mainDAO().getAll();
    ...
    private void filter(String query) {
        ...
        for(Notes eachnote: notesList){
            if(eachnote.getTitle().toLowerCase().contains(query.toLowerCase())
            || eachnote.getNote().toLowerCase().contains(query.toLowerCase())){
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode==101){
            if(resultCode == Activity.RESULT_OK){
                ...
                roomDB.mainDAO().insert(new_notes);
                notesList.clear();
                notesList.addAll(roomDB.mainDAO().getAll());
                notesListAdapter.notifyDataSetChanged();
            }
        }else if(requestCode == 102){
            if(resultCode == Activity.RESULT_OK){
                Notes new_notes = (Notes) data.getSerializableExtra("note");
                roomDB.mainDAO().update(new_notes.getID(), new_notes.getTitle(), new_notes.getNote());
                notesList.clear();
                notesList.addAll(roomDB.mainDAO().getAll());
                notesListAdapter.notifyDataSetChanged();
            }
        ...
        switch (item.getItemId()){
            case R.id.pin:
                if(selectedNote.isPinned()){
                    roomDB.mainDAO().pin(selectedNote.getID(),false);
                    Toast.makeText(this, "Unpinned", Toast.LENGTH_SHORT).show();
                }
                else{
                    roomDB.mainDAO().pin(selectedNote.getID(),true);
